...
│   │   └── ai-chat/
│   ├── store/
│   │   └── auth-store.ts       # Zustand: access token (memory), user info
│   ├── lib/
...
1. Login → access token stored in Zustand `authStore` (memory only, never localStorage).
...
- **All forms:** React Hook Form + Zod schema. No uncontrolled inputs.
